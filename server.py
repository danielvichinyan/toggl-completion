...
# (robust regardless of the working directory the MCP host launches us from)
...
class TogglClient:
    ...
    async def get_me(self) -> Dict[str, Any]:
        """Get the authenticated user's profile (includes timezone, default_workspace_id)."""
        if not self.session:
            raise RuntimeError("Client not initialized. Use async context manager.")

        url = f"{TOGGL_API_BASE}/me"
        async with self.session.get(url) as response:
            if response.status == 200:
                return await response.json()
            else:
                error_text = await response.text()
                raise Exception(f"Failed to get user profile: {response.status} - {error_text}")

    async def get_calendar_integrations(self) -> List[Dict[str, Any]]:
        """Get the user's connected calendar integrations (e.g. Outlook, Google)."""
        if not self.session:
            raise RuntimeError("Client not initialized. Use async context manager.")

        url = f"{TOGGL_API_BASE}/integrations/calendar"
        async with self.session.get(url) as response:
            if response.status == 200:
                return await response.json()
            else:
                error_text = await response.text()
                raise Exception(f"Failed to get calendar integrations: {response.status} - {error_text}")

    async def get_calendar_events(self, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """Get calendar events (from connected Outlook/Google calendars) already synced to Toggl.

        Reads events stored in Toggl's database for the given date range. Does NOT trigger a
        fresh fetch from the calendar provider (that is a separate, rate-limited endpoint).

        Args:
            start_date: Start date in YYYY-MM-DD format
            end_date: End date in YYYY-MM-DD format (upper boundary)
        """
        if not self.session:
            raise RuntimeError("Client not initialized. Use async context manager.")

        url = f"{TOGGL_API_BASE}/integrations/calendar/events"
        ...
        while True:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(f"Failed to get calendar events: {response.status} - {error_text}")
    ...
    async def create_time_entry(self, workspace_id: int, description: str, start: str, stop: str, project_id: Optional[int] = None) -> Dict[str, Any]:
        """Create a completed time entry with specific start and stop times.

        Args:
            workspace_id: ID of the workspace
            description: Description for the time entry
            start: Start time in ISO 8601 format (e.g., 2026-03-23T10:00:00Z)
            stop: Stop time in ISO 8601 format (e.g., 2026-03-23T13:00:00Z)
            project_id: Optional project ID to assign the time entry to
        """
        if not self.session:
            raise RuntimeError("Client not initialized. Use async context manager.")
        ...
        stop_dt = dt.fromisoformat(stop.replace("Z", "+00:00"))
        ...
        async with self.session.post(url, json=data) as response:
            if response.status in [200, 201]:
                ...
                error_text = await response.text()
                raise Exception(f"Failed to create time entry: {response.status} - {error_text}")
    ...
    async def delete_time_entry(self, workspace_id: int, time_entry_id: int) -> bool:
        ...
        async with self.session.delete(url) as response:
            if response.status in [200, 204]:
                ...
                error_text = await response.text()
                raise Exception(f"Failed to delete time entry: {response.status} - {error_text}")
        ...
        async with TogglClient(api_token) as client:
            ...
            for date in sorted(entries_by_date.keys()):
                ...
                for entry in entries_by_date[date]:
                    ...
                    result += f"  • {start_time} | {project_name_display} | {description} | {duration_str} | ID: {entry_id}\n"
...
@mcp.tool()
async def create_time_entry(description: str, start_time: str, stop_time: str, project_name: str = "") -> str:
    """
    Create a completed time entry with specific start and stop times.

    Use this to log past work — unlike start_timer, this creates a finished entry.

    Args:
        description: Description for the time entry (e.g., "#10163 - Implement test")
        start_time: Start time in ISO 8601 format (e.g., 2026-03-23T10:00:00+02:00)
        stop_time: Stop time in ISO 8601 format (e.g., 2026-03-23T13:00:00+02:00)
        project_name: Name of the project to assign the entry to (optional)
    ...
    try:
        ...
        async with TogglClient(api_token) as client:
            ...
            result = await client.create_time_entry(workspace_id, description, start_time, stop_time, project_id)
            ...
            response = f"**Time Entry Created!**\n\n"
            response += f"• **Description**: {description}\n"
            response += f"• **Project**: {project_name if project_name else 'No project'}\n"
            response += f"• **Start**: {start_time}\n"
            response += f"• **Stop**: {stop_time}\n"
            response += f"• **Duration**: {duration_str}\n"
...
@mcp.tool()
async def create_time_entries_bulk(entries_json: str, project_name: str = "") -> str:
    """
    Create multiple completed time entries in a single batch operation.

    Fetches workspace and project info ONCE, then creates all entries sequentially.
    Use this instead of calling create_time_entry repeatedly — each individual
    create_time_entry call costs 2 API requests (workspace + project lookup), so
    25 entries would burn 50 requests against the 30/hour quota. This tool costs
    2 requests total regardless of how many entries you create.

    Args:
        entries_json: JSON array of entries, each with "description", "start_time", "stop_time".
                      Example: '[{"description": "Meeting", "start_time": "2026-06-15T10:00:00+03:00", "stop_time": "2026-06-15T10:30:00+03:00"}]'
        project_name: Project to assign ALL entries to (optional). If omitted entries have no project.

    Returns a summary of created entries and any failures.
    """
    import json
    ...
    try:
        ...
        try:
            ...
            return f"Invalid JSON for entries_json: {e}"

        if not isinstance(entries, list) or not entries:
            return "entries_json must be a non-empty JSON array."

        async with TogglClient(api_token) as client:
            # Fetch workspace and project ONCE
            workspaces = await client.get_workspaces()
            if not workspaces:
                return "No workspaces found."
            workspace_id = workspaces[0].get("id")
            ...
            if project_name:
                ...
                if not project_id:
                    ...
                    return f"Project '{project_name}' not found. Available: {', '.join(available)}"
            ...
            for entry in entries:
                ...
                if not description or not start_time or not stop_time:
                    failed.append(f"Skipped (missing fields): {entry}")
                    continue

                try:
                    result = await client.create_time_entry(
                        workspace_id, description, start_time, stop_time, project_id
                    )
                    duration = result.get("duration", 0)
                    h, m = duration // 3600, (duration % 3600) // 60
                    created.append(f"✓ {description} ({h}h {m}m)")
                except Exception as e:
                    failed.append(f"✗ {description}: {e}")

            lines = [f"Bulk create: {len(created)} created, {len(failed)} failed\n"]
            lines += created
            if failed:
...
async def delete_time_entry(time_entry_id: int) -> str:
    ...
    try:
        ...
        async with TogglClient(api_token) as client:
            ...
            await client.delete_time_entry(workspace_id, time_entry_id)

            return f"**Time Entry Deleted!**\n\n• **Entry ID**: {time_entry_id}\n"
...
async def get_calendar_events(start_date: str, end_date: str) -> str:
    """
    Get meetings/events from your connected calendars (e.g. Outlook) as synced into Toggl.

    Use this to read calendar meetings for filling a timesheet — it replaces scraping the
    calendar in a browser. Returns events already synced to Toggl (it does NOT trigger a
    fresh provider fetch, so it does not consume the rate-limited calendar-sync quota).

    Times are converted to your Toggl account timezone and returned both as HH:MM and as a
    full ISO 8601 string with offset (ready to pass to create_time_entry).
    ...
    try:
        ...
        async with TogglClient(api_token) as client:
            ...
            tz_name = me.get("timezone") or "UTC"
            try:
                tz = ZoneInfo(tz_name)
            except Exception:
                tz = ZoneInfo("UTC")
            ...
            events = await client.get_calendar_events(start_date, end_date)

            if not events:
                return (f"No calendar events found from {start_date} to {end_date}. "
                        f"(If you expect events, they may not be synced to Toggl yet.)")
            ...
            for ev in events:
                ...
                if start_raw:
                    ...
                    local_date = s.strftime("%Y-%m-%d")
                    start_hm = s.strftime("%H:%M")
                    start_local_iso = s.isoformat()
                if end_raw:
                    e = datetime.fromisoformat(end_raw.replace("Z", "+00:00")).astimezone(tz)
                    end_hm = e.strftime("%H:%M")
                    end_local_iso = e.isoformat()

                by_date.setdefault(local_date, []).append({
                    "title": title, "all_day": all_day,
                    "start_hm": start_hm, "end_hm": end_hm,
                    "start_iso": start_local_iso, "end_iso": end_local_iso,
                })

            result = f"Calendar events ({start_date} to {end_date}, timezone {tz_name}):\n\n"
            for date in sorted(by_date.keys()):
                result += f"**{date}**\n"
                for ev in sorted(by_date[date], key=lambda x: x["start_hm"]):
                    if ev["all_day"]:
                        result += f"  • [all day] | {ev['title']}\n"
                    else:
                        result += (f"  • {ev['start_hm']}–{ev['end_hm']} | {ev['title']}\n"
                                   f"      start: {ev['start_iso']}  stop: {ev['end_iso']}\n")
                result += "\n"
...
@mcp.tool()
async def get_calendar_integrations() -> str:
    """
    List your connected calendar integrations in Toggl (e.g. Outlook, Google).

    Useful to confirm which calendar account is connected and whether auto-tracking is on.
    """
    try:
        api_token = get_api_token()

        async with TogglClient(api_token) as client:
            integrations = await client.get_calendar_integrations()

            if not integrations:
                return "No calendar integrations connected in Toggl."

            result = "Connected calendar integrations:\n\n"
            for i in integrations:
                result += (f"• **{i.get('provider', 'unknown')}** — {i.get('email', 'no email')}\n"
                           f"  - Auto-track events: {'Yes' if i.get('auto_track') else 'No'}\n"
                           f"  - Integration ID: {i.get('calendar_integration_id')}\n"
                           f"  - Error status: {i.get('error_status') or 'none'}\n\n")
            return result

    except ValueError as e:
        return f"Configuration error: {str(e)}"
    except Exception as e:
        return f"Error fetching calendar integrations: {str(e)}"
